...
一个不依赖网络的 Chrome / Edge Manifest V3 插件。直接打开本地 Markdown 文件即可获得带目录的阅读视图，打开文件夹则进入可搜索的本地文库；也支持在插件弹窗中实时编辑、自动保存草稿和复制渲染后的 HTML。
...
- **浏览文库**：打开本地文件夹，插件会生成面包屑、内容统计、搜索、排序和 Markdown 筛选视图；点击文档即可进入阅读模式。
...
- Mermaid 流程图、时序图、状态图等 fenced diagram；例如：
  ...
  渲染后的图表支持放大查看、浏览器全屏，以及下载 SVG 或 2× PNG。
